...
export default function AdminPuestos() {
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      await admin.remove("puestos", deleteDialog.item.nombre);
  ...
  }, [deleteDialog.item, loadPuestos, t]);
  ...
  return (
    <Container maxWidth={false} sx={{ py: 2, maxWidth: 1600 }}>
      ...
      <Dialog open={showForm} onClose={() => setShowForm(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ textTransform: "uppercase", fontWeight: 700 }}>{editingId ? "Editar Puesto" : "Nuevo Puesto"}</DialogTitle>
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, item: null })}>
        ...
        <DialogContent>
          <Typography>¿Eliminar el puesto <strong>{deleteDialog.item?.nombre}</strong>?</Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2, gap: 1 }}>
