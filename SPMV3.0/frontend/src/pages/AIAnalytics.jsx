...
const ALERT_COLORS = {
  warning: { bg: '#fffbeb', border: '#fcd34d', text: '#92400e', chip: 'warning' },
  breach: { bg: '#fef2f2', border: '#fca5a5', text: '#991b1b', chip: 'error' },
  escalated: { bg: '#f5f3ff', border: '#c4b5fd', text: '#5b21b6', chip: 'secondary' }
}
...
const MetricCard = ({ icon: Icon, label, value, color = '#1976d2', highlight = false }) => (
  <Paper
    elevation={0}
    sx={{
      p: 2.5,
      border: highlight ? `2px solid ${color}` : "1px solid #dce0e6",
      borderRadius: 2,
      bgcolor: highlight ? `${color}08` : "#ffffff"
    }}
  >
...
const ProgressCircle = ({ percentage, size = 140, color = '#1976d2', label }) => (
...
export default function AIAnalytics() {
  ...
  const getPipelineStatus = (pipelineStatus) => {
    if (!pipelineStatus) return { text: 'No disponible', color: '#94a3b8' }
    if (pipelineStatus === 'fitted' || pipelineStatus === 'ready') {
      return { text: 'Listo', color: '#10b981' }
    }
    if (pipelineStatus === 'training') {
      return { text: 'Entrenando', color: '#f59e0b' }
    }
    return { text: 'Pendiente', color: '#94a3b8' }
  }
  ...
  return (
    <Container maxWidth={false} sx={{ py: 2, px: "75px" }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          {(user?.rol === 'admin' || user?.rol === 'planificador' || user?.rol === 'administrador') && activeTab === 0 && (
            <Button
              ...
              sx={{ color: isTraining ? undefined : "#f59e0b", borderColor: isTraining ? undefined : "#f59e0b" }}
            >
        ...
        <Tabs
          ...
          sx={{
            ...
            "& .MuiTab-root": {
              ...
              "&:hover": { color: "#1976d2", bgcolor: "rgba(25, 118, 210, 0.04)" },
            },
      ...
      {activeTab === 0 && (
        <>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, 1fr)" }, gap: 2, mb: 3 }}>
            <Paper elevation={0} sx={{ p: 2.5, border: "1px solid #dce0e6", borderRadius: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Box sx={{ p: 1.5, bgcolor: "#f3e8ff", borderRadius: 2 }}>
                  <PsychologyIcon sx={{ fontSize: 24, color: "#7c3aed" }} />
                </Box>
                <Box sx={{ flex: 1 }}>
            ...
            <Paper elevation={0} sx={{ p: 2.5, border: "1px solid #dce0e6", borderRadius: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Box sx={{ p: 1.5, bgcolor: "#fef3c7", borderRadius: 2 }}>
                  <GpsFixedIcon sx={{ fontSize: 24, color: "#f59e0b" }} />
                </Box>
                <Box sx={{ flex: 1 }}>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr" }, gap: 3 }}>
            ...
            <Paper elevation={0} sx={{ border: "1px solid #dce0e6", borderRadius: 2 }}>
              <Box sx={{ p: 2.5, borderBottom: "1px solid #dce0e6" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  <BarChartIcon sx={{ color: "#ec4899" }} />
              ...
              <Box sx={{ p: 2.5, maxHeight: 380, overflowY: "auto" }}>
                {solicitudesPriorizadas.length === 0 ? (
                  <Box sx={{ textAlign: "center", py: 6 }}>
                    <PsychologyIcon sx={{ fontSize: 48, color: "#e9d5ff", mb: 1.5 }} />
                  ...
                  <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                    {solicitudesPriorizadas.map((sol, idx) => (
                      <Paper
                        key={sol.id || idx}
                        elevation={0}
                        sx={{ p: 2, bgcolor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 1.5 }}
                      >
                        <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                          <Box sx={{ flex: 1 }}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
                              <Typography variant="body2" fontWeight={600} color="#1f1f20">
                                #{sol.id}
                              </Typography>
                              {sol.criticidad && (
                                <Chip
                                  ...
                                  sx={{
                                    height: 20,
                                    fontSize: "0.65rem",
                                    bgcolor: sol.criticidad === 'alta' ? '#fee2e2' : sol.criticidad === 'media' ? '#fef3c7' : '#f1f5f9',
                                    color: sol.criticidad === 'alta' ? '#991b1b' : sol.criticidad === 'media' ? '#92400e' : '#475569'
                                  }}
                                />
                              )}
                            </Box>
                            ...
                            {sol.recomendacion && (
                              <Typography variant="caption" color="#1976d2" fontWeight={500} display="block" sx={{ mt: 0.5 }}>
                                {sol.recomendacion}
                              </Typography>
                            )}
                          </Box>
            ...
            <Paper elevation={0} sx={{ border: "1px solid #dce0e6", borderRadius: 2 }}>
              <Box sx={{ p: 2.5, borderBottom: "1px solid #dce0e6", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              <Box sx={{ p: 2.5, maxHeight: 380, overflowY: "auto" }}>
                {alertasIA.length === 0 ? (
                  <Box sx={{ textAlign: "center", py: 6 }}>
                    <CheckCircleIcon sx={{ fontSize: 48, color: "#86efac", mb: 1.5 }} />
                    <Typography variant="body2" color="text.secondary">
                    ...
                    {alertasIA.map((alerta, idx) => {
                      ...
                      return (
                        <Paper
                          ...
                          sx={{
                            p: 2,
                            bgcolor: isHigh ? '#fef2f2' : isMedium ? '#fffbeb' : '#eff6ff',
                            border: `1px solid ${isHigh ? '#fca5a5' : isMedium ? '#fcd34d' : '#93c5fd'}`,
                            borderRadius: 1.5
                          }}
                        >
                          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                            ...
                            />
                            <Box sx={{ flex: 1 }}>
                              <Typography variant="body2" fontWeight={500} color="#1f1f20">
                            ...
                            <Chip
                              ...
                              sx={{
                                height: 20,
                                fontSize: "0.65rem",
                                bgcolor: isHigh ? '#fee2e2' : isMedium ? '#fef3c7' : '#dbeafe',
                                color: isHigh ? '#991b1b' : isMedium ? '#92400e' : '#1e40af'
                              }}
                            />
                          </Box>
                        </Paper>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                          <Chip
                            ...
                            sx={{
                              height: 22,
                              fontSize: "0.7rem",
                              bgcolor: item.criticidad === 'alta' ? '#fee2e2' : item.criticidad === 'media' ? '#fef3c7' : '#f1f5f9',
                              color: item.criticidad === 'alta' ? '#991b1b' : item.criticidad === 'media' ? '#92400e' : '#475569'
                            }}
                          />
                          <Typography variant="body2" color="text.secondary">
                      ...
                      >
                        <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                          ...
                          <Button
                            ...
                            sx={{ color: "#10b981", ml: 2, whiteSpace: "nowrap" }}
                          >
      ...
      {/* Info adicional */}
      <Paper elevation={0} sx={{ mt: 3, p: 2, border: "1px solid #dce0e6", borderRadius: 2 }}>
