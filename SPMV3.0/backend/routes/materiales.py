...
def search_materiales():
    ...
    if q_grupo:
        filters.append("UPPER(grupo_articulos) LIKE UPPER(?)")
...
def get_grupos():
    ...
    if q_filter:
        query = """
            ...
            FROM materiales
            WHERE activo = 1 AND grupo_articulos IS NOT NULL
              AND UPPER(grupo_articulos) LIKE UPPER(?)
            ORDER BY grupo_articulos ASC
            LIMIT ?
        """
