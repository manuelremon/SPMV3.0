...
Use `test_production.py` to verify your production deployment:

```bash
# Configure credentials via environment variables (REQUIRED for security)
export TEST_ADMIN_PASSWORD='your-admin-password'

# Optional overrides (defaults to production URLs)
export TEST_BACKEND_URL='https://spmsystem2-0.onrender.com'
export TEST_FRONTEND_URL='https://spmv2-0.vercel.app'
export TEST_ADMIN_EMAIL='admin@spm.local'
...
**Security Note:** Never hardcode credentials in `test_production.py`. Always use environment variables to prevent accidental credential exposure in version control.
...
- [ ] Production tests ejecutados con `test_production.py`
